...
#include "lpc21isp_137x.h"   // if using propriatory serial port communication (customize attached lpc21isp.h)
#endif
...
static const int SectorTable_213x[] = { 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096,
                                        ...
                                        4096, 4096, 4096 };
static int SectorTable_246x[] = { 4096, 4096, 4096, 4096, 4096, 4096, 4096, 4096,
                                  32768, 32768, 32768, 32768, 32768, 32768, 32768, 32768,
                                  32768, 32768, 32768, 32768, 32768, 32768, 4096, 4096,
                                  4096, 4096, 4096 };
...
static LPC_DEVICE_TYPE LPCtypes[] =
{
  ...
  { 0x0603FB25, 2368, 512, 96, 27, 4096, SectorTable_246x },
  { 0x0703FF25, 2378, 512, 96, 27, 4096, SectorTable_246x },
  { 0x0603FF35, 2468, 512, 96, 27, 4096, SectorTable_246x },
};
...
static void ReadArguments(ISP_ENVIRONMENT *IspEnvironment, int argc, char *argv[])
{
    ...
    if(IspEnvironment->micro == PHILIPS_ARM)
    {
        ...
        {
            DebugPrintf( 1, "Invalid crystal frequency %s",argv[argc - 1]);
            exit(1);
        }
    }
}
...
DebugPrintf( 1, "Read size    = %d, \n", realsize);
DebugPrintf( 1, "tmpStringPos = %d, \n", tmpStringPos);
DebugPrintf( 1, "Answer = '%s'\n", Answer);
DebugPrintf( 1, "Temp   = '%s'\n", tmpString);
{
  int i;
for(i=0; i<realsize;i++)
{
  if (Answer[i] != tmpString[i])
    printf("\nError at [%d], Answer = %x, expected = %x", i, Answer[i], tmpString[i]);
}
if (realsize < tmpStringPos)
{
  for(i=realsize; i<tmpStringPos; i++)
    printf("\nMissing at [%d] expected = %x", i, tmpString[i]);
}
}
...
static void DumpString(int level, const void *b, size_t size, const char *fmt, ...)
{
    ...
    for( i = 0; i < size; i++)
    {
        ...
        {
            DebugPrintf( level, "%c",  s[i] );
        }
        else
        {
            DebugPrintf( level, "(%02X)", s[i] );
        }
    }
    DebugPrintf( level, "'\n" );
}
